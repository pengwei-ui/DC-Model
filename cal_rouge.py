...
#replace you perl path
_ROUGE_PATH = '/root/autodl-tmp/project/Abstract_Summarization_demo2/evaluation/ROUGE-RELEASE-1.5.5/'
...
def eval_rouge(dec_dir, ref_dir, Print=False):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        Rouge155.convert_summaries_to_rouge_format(
            dec_dir, join(tmp_dir, 'txt'))
        Rouge155.convert_summaries_to_rouge_format(
            ref_dir, join(tmp_dir, 'txt'))
        Rouge155.write_config_static(
            join(tmp_dir, 'txt'), dec_pattern,
            join(tmp_dir, 'txt'), ref_pattern,
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(args.ref):
        ...
        with open(args.ref, 'r') as f:
            for (i, line) in enumerate(f):
                ...
                if args.lower:
                    line = line.lower()
                with open(join(ref_dir, f"{i}.txt"), 'w') as f2:
        ...
        with open(args.hyp, 'r') as f:
            for (i, line) in enumerate(f):
                ...
                if args.lower:
                    line = line.lower()
                with open(join(hyp_dir, f"{i}.txt"), 'w') as f2:
                    for x in sent_tokenize(line):
                        print(x, file=f2)
    else:
        ref_dir = args.ref
    ...
    if args.python:
        ...
        for i in range(num):
            ref = open(join(ref_dir, f"{i}.txt"), 'r').read()
            hyp = open(join(hyp_dir, f"{i}.txt"), 'r').read()
            print(f"ref:{ref}")
            print(f"hyp:{hyp}")
            print("*"*30)
            ...
            # if scores['rougeLsum'].fmeasure < 30:
            #     print(i)
            cnt += 1

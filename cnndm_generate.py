...
def test(model, tokenizer, configs, dataset, epoch_index):
    ...
    for batch in tqdm(test_dataLoader):
        ...
        summaries = model.generate(input_ids=src_text["input_ids"].to(device),
                                   ...
                                   max_length=configs.test.gen_max_len + 2, num_beam_groups=configs.test.num_beams,
                                   diversity_penalty=0.3, num_beams=configs.test.num_beams,
    ...
    with open(f"generates.json", "w") as f:
        f.write(json.dumps(generates_))

    with open(f"gold_summarys.json", "w") as f:
...
if __name__ == '__main__':
    ...
    model = PW(dataset="cnndm", configs=configs,
               tokenizer=tokenizer, freeze=True,
               gpu_count=torch.cuda.device_count())
    # replace you model path
    model_paths = [f"/mnt/pw/Abstract_Summarization_demo2/cnndm_train_4412/train_generate.pth"]
    for index, model_path in enumerate(model_paths):
        checkpoint = torch.load(model_path, map_location='cuda:0')
        model.load_state_dict(checkpoint['model_state_dict'])
        model = model.to(device)
        result = test(model, tokenizer, configs, "cnndm", index + 1)
        print(result)
